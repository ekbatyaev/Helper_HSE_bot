...
async def formatting_excel(file_path):
    try:
        ...
        for weekday in weekdays:
            schedule += u"📚" + weekday + u"📚" + "\n\n"
            check = 0
            check += sum(len(column_data_lessons[index + p]) for p in range(8))
            print(check)
            if not check:
                schedule += "*В этот день пары нет*" + "\n\n"
            # check += sum(column_data_lessons[index + p] for p in range(8))
            # if column_data_lessons[index - 1] == "Занятия по дисциплинам дополнительного профиля MINOR":
            ...
            for i in range(8):
                if column_data_lessons[index + i] == '':
                    continue
                schedule += column_data_time[index + i] + ": " + "*" + column_data_lessons[index + i] + "*     _" + \
                            column_data_rooms[index + i] + "_" + "\n\n"
            schedule += "\n\n"
            index += 9
...
async def download_schedule():
    ...
    if os.path.exists(file_path):
        return await formatting_excel(file_path)
    else:
        print('Файл не найден после загрузки.')

async def send_schedule():
    schedule = await download_schedule()
    return schedule
...
def get_answer(pattern, page_id):
    ...
    message_for_user ='Схожие вопросы: \n\n'
    for i in range(0, 3):
...
item2 = types.InlineKeyboardButton('Обратиться в ККО', callback_data= 'support')
...
item4 = types.InlineKeyboardButton("Переформулировать вопрос", callback_data = 'rephrase')
...
def feedback_message(message):
    ...
    if message.chat.id == chat_information[2]:
        feedback_message = bot.send_message(chat_information[2], "Пришлите id пользователя, которому отвечаете.", parse_mode="Markdown")
        handle_message(feedback_message.message_id)
...
def access_message(message):
    page_id = "0030e2cc086b4a9880ab236eb8228aa0"
    get_chat_info(page_id)
    feedback_message = bot.send_message(message.from_user.id,  "На данной странице ты можешь отправить запрос в студсовете, а именно в Комитет Качества Образования.\n"
                                                             "Это стоит делать в случае конфликтной ситуации, не типового вопроса, не соответствии коэффициентов ПУДУ и т.д.", reply_markup = markup5, parse_mode="Markdown")
    handle_message(feedback_message.message_id)
...
def first_message(message):
    if message.data == 'Начать работу':
        bot_first_message = bot.edit_message_text("Выбери свой факультет: ", message.from_user.id, last_message_id,  reply_markup=markup1, parse_mode = "Markdown")
...
def problem_types(message):
    bot_problem_types = bot.edit_message_text('Вы хотите задать вопрос связанный с выбранным факультетом?\n'
                                           'Пример: Получить справку о месте обучения КНТ.', message.from_user.id, last_message_id,  reply_markup=markup2, parse_mode = "Markdown")
...
def back_fac_it(message):
    if message.data == 'back':
        bot_back_fac_it_message = bot.edit_message_text("Выбери свой факультет: ",
                                                  message.from_user.id, last_message_id, reply_markup=markup1,
                                                  parse_mode="Markdown")
        handle_message(bot_back_fac_it_message.message_id)
        bot.set_state(message.from_user.id, MainStates.problem_types)
    elif message.data == 'ask_question':
        bot_go_fac_it_message = bot.edit_message_text("Ваш вопрос: \n",
                                                        message.from_user.id, last_message_id,
                                                        parse_mode="Markdown")
        ...
        page_id = "0030e2cc086b4a9880ab236eb8228aa0"
        get_chat_info(page_id)
        feedback_message = bot.edit_message_text("На данной странице ты можешь отправить запрос в студсовете, а именно в Комитет Качества Образования.\n"
                                            "Это стоит делать в случае конфликтной ситуации, не типового вопроса, не соответствии коэффициентов ПУДУ и т.д.",
                                                        message.from_user.id, last_message_id, reply_markup = markup5,
                                                        parse_mode="Markdown")
        handle_message(feedback_message.message_id)
        bot.set_state(message.from_user.id, KKO_group.back_request)
    elif message.data == 'answer_1':
        question_answer = bot.edit_message_text("Ответ на вопрос №1: \n" + answer[0],
                                                        message.from_user.id, last_message_id, reply_markup = markup3,
                                                        parse_mode="Markdown")
        handle_message(question_answer.message_id)
        bot.set_state(message.from_user.id, Back_fac.back_fac_it)
    elif message.data == 'answer_2':
        question_answer = bot.edit_message_text("Ответ на вопрос №2: \n" + answer[1],
                                                        message.from_user.id, last_message_id, reply_markup = markup3,
                                                        parse_mode="Markdown")
        handle_message(question_answer.message_id)
        bot.set_state(message.from_user.id, Back_fac.back_fac_it)
    elif message.data == 'answer_3':
        question_answer = bot.edit_message_text("Ответ на вопрос №3: \n" + answer[2],
                                                        message.from_user.id, last_message_id, reply_markup = markup3,
                                                        parse_mode="Markdown")
        handle_message(question_answer.message_id)
        bot.set_state(message.from_user.id, Back_fac.back_fac_it)
    elif message.data == 'rephrase':
        rephrase_mess = bot.edit_message_text("Ваш вопрос: ",
                                                        message.from_user.id, last_message_id,
                                                        parse_mode="Markdown")
        handle_message(rephrase_mess)
        bot.set_state(message.from_user.id, Faculties_types.fac_it)

@bot.callback_query_handler(state = Back_fac.back_fac_gum, func = lambda callback: True)
def back_fac_gum(message):
    if message.data == 'back':
        bot_back_fac_gum_message = bot.edit_message_text("Выбери свой факультет: ",
                                                  message.from_user.id, last_message_id, reply_markup=markup1,
                                                  parse_mode="Markdown")
        handle_message(bot_back_fac_gum_message.message_id)
        bot.set_state(message.from_user.id, MainStates.problem_types)
    elif message.data == 'ask_question':
        bot_go_fac_gum_message = bot.edit_message_text("Ваш вопрос: \n",
                                                        message.from_user.id, last_message_id,
                                                        parse_mode="Markdown")
...
def back_fac_man(message):
    if message.data == 'back':
        bot_back_fac_man_message = bot.edit_message_text("Выбери свой факультет: ",
                                                  message.from_user.id, last_message_id, reply_markup=markup1,
                                                  parse_mode="Markdown")
        handle_message(bot_back_fac_man_message.message_id)
        bot.set_state(message.from_user.id, MainStates.problem_types)
    elif message.data == 'ask_question':
        bot_go_fac_man_message = bot.edit_message_text("Ваш вопрос: \n",
                                                        message.from_user.id, last_message_id,
                                                        parse_mode="Markdown")
...
def back_fac_law(message):
    ...
    if message.data == 'back':
        bot_back_fac_law_message = bot.edit_message_text("Выбери свой факультет: ",
                                                  message.from_user.id, last_message_id, reply_markup=markup1,
                                                  parse_mode="Markdown")
        handle_message(bot_back_fac_law_message.message_id)
        bot.set_state(message.from_user.id, MainStates.problem_types)
    elif message.data == 'ask_question':
        bot_go_fac_law_message = bot.edit_message_text("Ваш вопрос: \n",
                                                        message.from_user.id, last_message_id,
                                                        parse_mode="Markdown")
...
def back_fac_econ(message):
    if message.data == 'back':
        bot_back_fac_econ_message = bot.edit_message_text("Выбери свой факультет: ",
                                                  message.from_user.id, last_message_id, reply_markup=markup1,
                                                  parse_mode="Markdown")
        handle_message(bot_back_fac_econ_message.message_id)
        bot.set_state(message.from_user.id, MainStates.problem_types)
    elif message.data == 'ask_question':
        bot_go_fac_econ_message = bot.edit_message_text("Ваш вопрос: \n",
                                                        message.from_user.id, last_message_id,
                                                        parse_mode="Markdown")
...
def fac_it(message):
    ...
    answer_bot_message = bot.edit_message_text(get_answer(message.text, page_id_fac), message.from_user.id, last_message_id, reply_markup=markup4, parse_mode = "Markdown")
    handle_message(answer_bot_message.message_id)
    bot.set_state(message.from_user.id, Back_fac.back_fac_it)
...
def get_id(message):
    ...
    request_message = bot.edit_message_text("Пришлите ответ на запрос студента в формате одного сообщения.",
                                            chat_information[2], last_message_id, parse_mode = "Markdown")
    handle_message(request_message.message_id)
...
def check_answer(message):
    ...
    check_answer = bot.edit_message_text("Ваш ответ: \n\n" + message.text + '\n\n' + "ID студента: " + str(send_user_id), chat_information[2],
                                         last_message_id, reply_markup = markup8,  parse_mode = "Markdown")
    handle_message(check_answer.message_id)
...
def status_answer(message):
    if message.data == 'send_answer':
        status_message = bot.send_message(chat_information[2], "Ваш ответ на запрос отправлен.", parse_mode="Markdown")
        bot.send_message(send_user_id, "Ответ от ККО: \n\n" + ans_req_message, reply_markup=markup7, parse_mode="Markdown")
        ...
        feedback_message = bot.edit_message_text("Пришлите id пользователя, которому отвечаете.",
                                            chat_information[2], last_message_id, parse_mode = "Markdown")
        handle_message(feedback_message.message_id)
        bot.set_state(message.from_user.id, KKO_group.get_id, chat_information[2])
...
def feedback_answer(message):
    if message.data =='accepted':
        bot.send_message(message.from_user.id, "Был рад вам помочь!" )
    elif message.data == 'not_accepted':
        bot.send_message(message.from_user.id, "Вы можете обратиться к одному из сотрудников ККО: \n\n" + chat_information[1])
...
def back_request_message(message):
    if message.data == 'write_request':
        explain_message = bot.edit_message_text("В твоем запросе ты должен указать свое ФИО, факультет, учебную группу.\n"
                                                 "Твой запрос: ",
                                            message.from_user.id, last_message_id, parse_mode = "Markdown")
        handle_message(explain_message.message_id)
        bot.set_state(message.from_user.id, KKO_group.correct_request)
    elif message.data == 'back':
        bot_first_message = bot.edit_message_text("Выбери свой факультет: ",
                                            message.from_user.id, last_message_id, reply_markup = markup1, parse_mode = "Markdown")
        handle_message(bot_first_message.message_id)
        bot.set_state(message.from_user.id, MainStates.problem_types)
    elif message.data == 'send_request':
        bot.send_message(chat_information[2], "Запрос от студента: \n\n" + request_message +"\n\n"+"ID студента: \n" + str(message.from_user.id), parse_mode="Markdown")
        bot.send_message(message.from_user.id, "Ваш запрос направлен в ККО", parse_mode="Markdown")


@bot.message_handler(state=KKO_group.correct_request)
def request_check(message):
    put_request_message(message.text)
    request_message =bot.edit_message_text("Ваш запрос выглядит так: \n\n" + message.text + "\n\n" + "Проверьте, написали ли вы: учебную группу, факультет, ФИО.",
                                            message.from_user.id, last_message_id, reply_markup = markup6, parse_mode = "Markdown")
    handle_message(request_message.message_id)
    bot.set_state(message.from_user.id, KKO_group.back_request)

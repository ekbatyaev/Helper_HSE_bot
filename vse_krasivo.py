...
def first_message(message):
    if message.data == 'Начать работу':
        bot_first_message = bot.edit_message_text(f"_Выбери свой факультет: _", message.from_user.id, last_message_id,  reply_markup=markup1, parse_mode = "Markdown")
...
def check_answer(message):
    bot.delete_message(chat_information[2], last_message_id)
...
def back_request_message(message):
    ...
    if message.data == 'write_request':
        ...
        bot.send_message(chat_information[2], f"_Запрос от студента: _" + "\n\n" + f"{request_message}" + "\n\n" + f"*ID студента: *" + "\n" + str(message.from_user.id), parse_mode="Markdown")
        bot.send_message(message.from_user.id, f"*Ваш запрос *" + f"_направлен _" + f"*в ККО*", parse_mode="Markdown")
